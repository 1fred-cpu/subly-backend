...
export class AuthService {
    ...
    async register(dto: RegisterDto) {
        const { companyName, password, ownerEmail, ownerName } = dto;
        ...
        try {
            ...
            // If owner user exists, throw conflict exception
            if (existingOwner) {
                throw new ConflictException(
            ...
            // Create a new owner user instance
            const newOwner = await queryRunner.manager.create(User, {
    ...
    async signIn(
        ...
        try {
            ...
            const accessTokenExpiresAt = addMinutes(new Date(), 60); // 60 minutes
    ...
    async verifyEmail(token: string, ipAddress?: string, userAgent?: string) {
        ...
        try {
            ...
            // Dashboard url link
            const dashboardUrlLink = `${this.configService.get<string>(
                "FRONTEND_URL"
            )}/dashboard`;
            ...
            this.logger.error(
                `Error during email verification: ${error.stack}`
            );
        ...
        try {
            ...
            return await this.dataSource.transaction(
                async (manager: EntityManager) => {
                    ...
                    if (
                        isBefore(
                            session.refreshTokenExpiresAt as Date,
                            new Date()
                        )
                    ) {
                        throw new UnauthorizedException(
                    ...
                    const newAccessTokenExpiry = addMinutes(new Date(), 60); // 15 minutes
                    ...
                    if (
                        ...
                        updatedRefreshToken =
                            this.jwtHelper.generateRefreshToken({
                                userId: session.user.id
                            });
                        updatedRefreshToken = await hash(
                            updatedRefreshToken,
                            10
                        );
    ...
    async requestPasswordReset(email: string): Promise<{ message: string }> {
        ...
        try {
            ...
            // Reset url link
            const resetUrlLink = `${this.configService.get<string>(
                "FRONTEND_URL"
